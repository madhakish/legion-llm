...
def make_skill(name:, ns:, trigger: :on_demand, words: [], follows: nil, file_triggers: [])
  klass = Class.new(Legion::LLM::Skills::Base)
  klass.skill_name(name); klass.namespace(ns); klass.trigger(trigger)
  ...
  klass.define_method(:s) { |context: {}| Legion::LLM::Skills::StepResult.build(inject: 's') }
  klass.steps(:s)
  klass
end
...
RSpec.describe Legion::LLM::Skills::Registry do
  ...
  describe '.register / .find' do
    it 'registers and retrieves by namespace:name key' do
      sk = make_skill(name: 'skill-a', ns: 'test')
  ...
  describe '.all' do
    it 'returns skills in insertion order' do
      a = make_skill(name: 'a', ns: 'test')
      b = make_skill(name: 'b', ns: 'test')
      described_class.register(a); described_class.register(b)
  ...
  describe '.by_trigger' do
    it 'filters by trigger type' do
      on_demand = make_skill(name: 'od', ns: 'test', trigger: :on_demand)
      auto      = make_skill(name: 'au', ns: 'test', trigger: :auto)
      described_class.register(on_demand); described_class.register(auto)
  ...
  describe 'duplicate registration' do
    it 'replaces previous and logs warning (last wins)' do
      v1 = make_skill(name: 'sk', ns: 'test')
      v2 = make_skill(name: 'sk', ns: 'test')
  ...
  describe 'cycle detection' do
    it 'raises InvalidSkill when chain loops back' do
      ca = make_skill(name: 'ca', ns: 'test', follows: 'test:cb')
      cb = make_skill(name: 'cb', ns: 'test', follows: 'test:ca')
  ...
  describe 'trigger word index' do
    it 'builds reverse index' do
      sk = make_skill(name: 'sk', ns: 'test', words: %w[foo bar])
      described_class.register(sk)
  ...
  describe 'file trigger index' do
    it 'tracks skills with file_change_triggers' do
      sk = make_skill(name: 'sk', ns: 'test', file_triggers: ['*.rb'])
      described_class.register(sk)
